...
class ModelPerformanceTracker:
    ...
    def record_query_result(self, model: str, success: bool, response_time: float, 
                           was_selected: bool, consensus_score: float = 0.0,
        ...
        if len(model_data['recent_performance']) > self.thresholds['evaluation_window']:
            model_data['recent_performance'] = model_data['recent_performance'][-self.thresholds['evaluation_window']:]
    ...
    def evaluate_model_performance(self, model: str) -> Dict:
        ...
        avg_response_time = np.mean([r['response_time'] for r in recent if r['success']])
        
        # Calculate composite score (0-1)
        success_score = recent_success_rate
        selection_score = min(recent_selection_rate / 0.2, 1.0)  # Normalize to expected 20% selection
    ...
    def get_model_recommendations(self, current_models: List[str], 
    ...
    def get_performance_summary(self) -> str:
        ...
        for model, data in self.performance_data['models'].items():
            ...
            if data['total_queries'] > 0:
                success_rate = (data['successful_queries'] / data['total_queries']) * 100
...
class AdaptiveModelManager:
    ...
    def detect_system_config(self) -> Dict:
        ...
        try:
            ...
            self.logger.info(f"Detected {total_ram_gb:.1f}GB RAM, using {config['max_models']} models max")
            return config
    ...
    async def optimize_models(self, current_models: List[str]) -> List[str]:
        ...
        if recommendations['remove']:
            self.logger.info(f"Removing underperforming models: {recommendations['remove']}")
        ...
        if len(current_models) > self.system_config['max_models']:
            ...
            current_models = [m[0] for m in sorted_models[:self.system_config['max_models']]]
            
            self.logger.info(f"Optimized to {len(current_models)} models: {current_models}")
    ...
    async def preload_models(self, models: List[str]):
        ...
        self.logger.info(f"Preloading {len(models)} models with {self.system_config['stagger_delay']}s delay")
    ...
    def get_optimized_timeout(self, model: str, base_timeout: Optional[int] = None) -> int:
        ...
        elif evaluation['status'] == 'new' or evaluation['status'] == 'insufficient_data':

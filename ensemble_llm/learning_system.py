    ...
    def find_similar_query(
        ...
        try:
            ...
            if max_similarity >= threshold:
                ...
                self.logger.info(
                    f"Found similar query with {max_similarity:.2f} similarity"
                )
...
class ModelOptimizer:
    ...
    def update_model_params(
        ...
        if success and response_quality > 0.7:
            ...
            for param_key in ["temperature", "top_p"]:
                if param_key in params:
                    current_val = model_opt["optimal_params"].get(
        ...
        if len(model_opt["quality_scores"]) > 10:
            avg_quality = sum(model_opt["quality_scores"]) / len(
                model_opt["quality_scores"]
            )

            if avg_quality > 0.8:
                model_opt["optimal_params"]["num_predict"] = min(
                    768, params.get("num_predict", 512) + 50
                )
            elif avg_quality < 0.5:
                model_opt["optimal_params"]["num_predict"] = max(
                    256, params.get("num_predict", 512) - 50
                )
    ...
    def get_model_confidence(self, model: str) -> float:
        ...
        avg_quality = (
            sum(model_opt["quality_scores"]) / len(model_opt["quality_scores"])
            if model_opt["quality_scores"]
            else 0.5
        )

        # Recent performance matters more
        recent_success = sum(list(model_opt["success_rate"])[-10:]) / min(
            10, len(model_opt["success_rate"])
        )
...
class QueryPatternLearner:
    ...
    def learn_from_query(
        ...
        if query_type:
            ...
            pref["count"] += 1
            pref["avg_quality"] = (
                pref["avg_quality"] * (pref["count"] - 1) + response_quality
            ) / pref["count"]

    def get_recommended_models(
        self, query: str, query_type: Optional[str], available_models: List[str]
    ) -> List[str]:
        ...
        if query_type and query_type in self.patterns["model_preferences"]:
            ...
            for model in available_models:
                if model in prefs:
                    score = prefs[model]["avg_quality"] * np.log1p(
                        prefs[model]["count"]
                    )
...
class SmartEnsembleOrchestrator:
    ...
    def __init__(self, data_dir: str = "smart_data"):
        ...
        self.model_optimizer = ModelOptimizer(
            optimizer_dir=str(self.data_dir / "optimizers")
        )
        self.pattern_learner = QueryPatternLearner(
            pattern_dir=str(self.data_dir / "patterns")
        )
    ...
    async def check_cache(self, query: str) -> Optional[Tuple[str, Dict]]:
        ...
        if cached:
            ...
            if (
                not response
                or "all_failed" in metadata
                or metadata.get("error")
                or response == ERROR_MESSAGES.get("all_failed", "All models failed")
            ):

                # Remove bad cache entry
                self.logger.warning(
                    f"Removing invalid cache entry for query: {query[:50]}..."
                )
            ...
            self.logger.info(
                f"Cache hit (exact match) - {self.session_data['cache_hits']} total hits"
            )
        ...
        if similar_result:
            ...
            if cached:
                ...
                if not response or "all_failed" in metadata or metadata.get("error"):
                    return None

                self.session_data["cache_hits"] += 1
                self.logger.info(
                    f"Cache hit (similarity: {similarity:.2f}) - {self.session_data['cache_hits']} total hits"
                )
    ...
    def update_learning(
        self, query: str, response: str, metadata: Dict, model_performances: List[Dict]
    ):
        """Update all learning components"""

        if (
            response
            and not metadata.get("all_failed")
            and not metadata.get("error")
            and response != ERROR_MESSAGES.get("all_failed", "All models failed")
        ):
    ...
    def get_models(
        ...
        sorted_models = sorted(
            model_confidences.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def get_performance_insights(self) -> Dict:
        ...
        insights = {
            ...
            "cache_hit_rate": (
                self.session_data["cache_hits"]
                / max(1, self.session_data["total_queries"])
            )
            * 100,
        ...
        for model in self.model_optimizer.optimizations["models"]:
            insights["model_confidences"][model] = (
                self.model_optimizer.get_model_confidence(model)
            )

        # Add top models by query type
        for query_type, prefs in self.pattern_learner.patterns[
            "model_preferences"
        ].items():
            sorted_prefs = sorted(
                prefs.items(), key=lambda x: x[1]["avg_quality"], reverse=True
            )
            insights["top_models_by_type"][query_type] = [
                m[0] for m in sorted_prefs[:3]
            ]
...
class PrecomputeManager:
    ...
    def load_embeddings(self) -> Dict:
        ...
        return {"common_queries": {}, "model_signatures": {}, "quick_responses": {}}
    ...
    async def precompute_common_queries(
        self, models: List[str], common_queries: List[str]
    ):
        ...
        for cache_file in cache_path.glob("*.pkl"):
            try:
                ...
                if (
                    ...
                    or "all_failed" in metadata
                    or metadata.get("error")
                    or "All models failed" in response
                ):

...
class EnsembleLLM:
    def __init__(
        ...
        self.enable_web_search = (
            ...
            else FEATURES["web_search"]
        )
        self.adaptive_mode = (
            adaptive_mode if adaptive_mode is not None else FEATURES["adaptive_models"]
        )
        ...
        if self.adaptive_mode and FEATURES["performance_tracking"]:
            self.performance_tracker = ModelPerformanceTracker(
                data_dir=TRACKING_CONFIG["data_dir"]
            )
            self.model_manager = AdaptiveModelManager(self.host)
            self.logger.info(
                "Adaptive mode enabled - models will be optimized based on performance"
            )
        else:
        ...
        self.logger.info(
            f"Web search: {'Enabled' if self.enable_web_search else 'Disabled'}"
        )
        self.logger.info(
            f"Adaptive optimization: {'Enabled' if self.adaptive_mode else 'Disabled'}"
        )
        ...
        if smart_learning:
            ...
            self.precompute_manager = PrecomputeManager(
                precompute_dir=str(
                    Path(TRACKING_CONFIG.get("smart_data_dir", "smart_data"))
                    / "precompute"
                )
            )
            self.logger.info(
                "Smart learning system enabled - queries will be optimized over time"
            )

            # Load performance insights
            insights = self.orchestrator.get_performance_insights()
            self.logger.info(
                f"Loaded smart data: {insights['total_queries']} previous queries, "
        ...
        if speed_mode in SPEED_OPTIMIZED_MODELS:
            ...
            self.models = [
                m for m in available_models if m in (models or DEFAULT_MODELS)
            ]
            if not self.models:
                self.models = models or DEFAULT_MODELS
    ...
    async def initialize(self):
        ...
        if self.speed_mode in ["turbo", "fast"]:
            await self.model_warmup.parallel_warmup(self.models, max_concurrent=2)
        ...
        if self.model_manager and FEATURES["staggered_starts"]:
            ...
            if FEATURES["model_rotation"]:
                ...
                self.logger.info(
                    SUCCESS_MESSAGES["optimization_complete"].format(
        ...
        if self.smart_learning and self.orchestrator:
            ...
            if self.precompute_manager:
                await self.precompute_manager.precompute_common_queries(
                    self.models[:3], common_queries  # Only precompute for top 3 models
                )
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.host}{OLLAMA_ENDPOINTS['tags']}"
                ) as response:
                    if response.status == 200:
                        data = await response.json()
                        available_models = {
                            m["name"]: m["name"] for m in data.get("models", [])
                        }
                        ...
                        for model in models:
                            if model in available_models:
                                ...
                                if latest_tag in available_models:
                                    self.logger.info(
                                        f"Resolved '{model}' to '{latest_tag}'"
                                    )
                                    resolved_models.append(latest_tag)
                                else:
                                    self.logger.warning(
                                        ERROR_MESSAGES["invalid_model"].format(
                                            model=model
                                        )
                                    )
                                    resolved_models.append(model)
    ...
    async def check_model_availability(
        ...
        if (
            ...
            and self.model_availability_cache
        ):
    ...
    async def query_model_fast(self, model: str, prompt: str) -> Dict:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            timeout = FAST_MODEL_CONFIGS.get(model, {}).get(
                "timeout", self.speed_profile["timeout"]
            )
    ...
    async def enhance_prompt_with_web_search(self, prompt: str) -> Tuple[str, bool]:
        ...
        if "?" in prompt and any(
            ...
            for word in ["who is", "what is", "when is", "where is"]
        ):
            if any(
                word in prompt_lower
                for word in ["president", "ceo", "champion", "winner"]
            ):
                needs_search = True
    ...
    async def query_model_optimized(
        ...
        if self.smart_learning and self.orchestrator:
            ...
            if self.precompute_manager:
                precomputed = self.precompute_manager.get_precomputed_response(
                    prompt, model
                )
                if precomputed:
        ...
        if self.model_manager:
            ...
            timeout = model_config.get(
                "timeout", SYSTEM_CONFIGS["24GB"]["timeout_base"]
            )
        ...
        if self.enable_web_search and not hasattr(self, "_current_web_context"):
            enhanced_prompt, used_web_search = (
                await self.enhance_prompt_with_web_search(prompt)
            )
            if used_web_search:
        ...
        for attempt in range(max_retries):
            try:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        if (
                            ...
                            and await self.detect_uncertainty(response_text)
                        ):

                            self.logger.info(
                                f"Model {model} seems uncertain, adding web search context..."
                            )
                            enhanced_prompt, used_web_search = (
                                await self.enhance_prompt_with_web_search(prompt)
                            )
                        ...
                        if attempt < max_retries - 1 and FEATURES["auto_retry"]:
                            await asyncio.sleep(
                                ENSEMBLE_CONFIG["retry_delay"] ** attempt
                            )
                            continue
                ...
                if FEATURES["verbose_errors"]:
                    self.logger.error(
                        f"Client error for model {model}: {type(e).__name__}: {str(e)}"
                    )
                ...
                if FEATURES["verbose_errors"]:
                    self.logger.error(
                        f"Unexpected error for model {model}: {type(e).__name__}: {str(e)}"
                    )
    ...
    async def query_all_models_optimized(self, prompt: str) -> List[Dict]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if self.model_manager:
                stagger_delay = self.model_manager.system_config.get(
                    "stagger_delay", ENSEMBLE_CONFIG["stagger_delays"]["24GB"]
                )
            else:
    ...
    def weighted_voting(self, responses: List[Dict]) -> Tuple[str, Dict]:
        ...
        valid_responses = [
            r for r in responses if r["success"] and len(r.get("response", "")) > 10
        ]
        ...
        for i, response in enumerate(valid_responses):
            ...
            web_bonus = (
                ENSEMBLE_CONFIG["web_bonus"]
                if response.get("used_web_search", False)
                else 1.0
            )
            ...
            if self.performance_tracker and FEATURES["performance_tracking"]:
                model_eval = self.performance_tracker.evaluate_model_performance(
                    response["model"]
                )
        ...
        final_scores = (
            consensus_scores * consensus_weight + quality_scores * quality_weight
        )
    ...
    async def ensemble_query(
        ...
        if self.smart_learning and self.orchestrator:
            ...
            if cached_result:
                ...
                if response and not metadata.get("all_failed"):
                    ...
                    self.logger.warning(
                        "Invalid cached response, proceeding with fresh query"
                    )
        ...
        if (
            self.adaptive_mode
            and FEATURES["model_rotation"]
            and self.query_count % self.optimization_interval == 0
        ):
        ...
        self.logger.info(
            f"Starting query in {self.speed_mode} mode with {len(self.models)} models"
        )
        ...
        if (
            ...
            and FEATURES["performance_tracking"]
            and not metadata.get("all_failed", False)
        ):
            ...
            for response in responses:
                if response["success"]:
                    ...
                    model_scores = metadata.get("all_scores", {}).get(
                        response["model"], {}
                    )
        ...
        if (
            self.performance_tracker
            and self.query_count % TRACKING_CONFIG["save_interval"] == 0
        ):
        ...
        self.logger.info(
            f"Query completed in {total_time:.2f}s - Selected: {metadata.get('selected_model', 'N/A')}"
        )
                    ...
                    model_performances.append(
                        {
                            ...
                            "response_time": response.get("response_time", 0),
                            "params": (
                                optimal_params if "optimal_params" in locals() else {}
                            ),
                ...
                for response in responses:
                    if response["success"]:
                        ...
                        self.performance_logger.log_model_query(
                            ...
                            was_selected=(
                                response["model"] == metadata.get("selected_model")
                            ),
                            quality_score=model_scores.get("quality", 0),
                            consensus_score=model_scores.get("consensus", 0),
                        )
    ...
    def display_verbose_output(
        self, responses: List[Dict], metadata: Dict, total_time: float
    ):
        ...
        if "all_scores" in metadata and len(metadata["all_scores"]) > 1:
            ...
            for model, scores in sorted_models:
                ...
                print(
                    f"   {web_indicator} {model:30} | Final: {scores['final']:.3f} | "
        ...
        if self.verbose_logging and hasattr(self, "performance_logger"):
            ...
            if hasattr(self, "verbose_logger"):
                stats = {
                    ...
                    "cache_hits": (
                        self.orchestrator.session_data["cache_hits"]
                        if self.orchestrator
                        else 0
                    ),
                    "avg_response_time": (
                        self.orchestrator.session_data["avg_response_time"]
                        if self.orchestrator
                        else 0
                    ),
...
async def main():
    ...
    parser.add_argument(
        "--models", nargs="+", default=DEFAULT_MODELS, help="Models to use in ensemble"
    )
    parser.add_argument(
        "--verbose",
        "-v",
    ...
    parser.add_argument(
        "--no-adaptive", action="store_true", help="Disable adaptive model optimization"
    )
    parser.add_argument(
        "--performance-report",
        "-p",
    ...
    parser.add_argument(
        "--no-smart", action="store_true", help="Disable smart learning system"
    )
    parser.add_argument(
        "--clear-cache", action="store_true", help="Clear smart cache and start fresh"
    )
    parser.add_argument(
        "--insights", action="store_true", help="Show smart learning insights"
    )
    parser.add_argument(
        "--speed",
        choices=["turbo", "fast", "balanced", "quality"],
        default="balanced",
        help="Speed optimization mode",
    )
    parser.add_argument(
        "--warmup", action="store_true", help="Warmup models before querying"
    )
    parser.add_argument(
        "--clear-failed-cache",
        action="store_true",
    ...
    parser.add_argument(
        "--tail-log", action="store_true", help="Tail the live log file"
    )
    parser.add_argument(
        "--view-log", action="store_true", help="View today's verbose log"
    )
    parser.add_argument("--log-stats", action="store_true", help="Show log statistics")
    parser.add_argument(
        "--optimize-windows",
        action="store_true",
        help="Apply Windows GPU optimizations",
    )
    ...
    if args.insights:
        ensemble = EnsembleLLM(models=args.models, smart_learning=True)

...
"""
    ...
    def log_query_start(self, query_id: int, prompt: str, models: List[str], 
                       speed_mode: str = None, web_search: bool = False):
...
"""
        ...
        if 'all_scores' in voting_data:
            ...
            for i, (model, scores) in enumerate(sorted_scores, 1):
                selected = "**SELECTED**" if model == voting_data.get('selected_model') else ""
                content += f"""
  {i}. {model} {selected}
...
"""
    ...
    def log_model_rotation(self, removed_models: List[str], added_models: List[str], 
                          reasons: Dict[str, str], performance_data: Dict = None):
...
"""
        ...
        for model in removed_models:
            ...
            if performance_data and model in performance_data:
                ...
                content += f"      - Selection Rate: {perf.get('selection_rate', 0):.1%}\n"
...
class ModelPerformanceLogger:
    ...
    def __init__(self, log_dir: str = "logs"):
        ...
        self.daily_summary_log = self.log_dir / f"daily_summary_{datetime.now().strftime('%Y%m%d')}.json"
        
        # In-memory buffer for performance data
        self.performance_buffer = deque(maxlen=1000)
        
    def log_model_query(self, model: str, success: bool, response_time: float,
                       query_type: str = None, was_selected: bool = False,
                       quality_score: float = 0, consensus_score: float = 0):
    ...
    def generate_daily_summary(self):
            ...
            with open(self.performance_log, 'r') as f:
                for line in f:
                    try:
                        ...
                        model_stats['total_quality_score'] += entry.get('quality_score', 0)
        ...
        for model, stats in summary['models'].items():
            if stats['total_queries'] > 0:
                ...
                stats['selection_rate'] = stats['times_selected'] / stats['total_queries']
                stats['avg_quality_score'] = stats['total_quality_score'] / stats['total_queries']

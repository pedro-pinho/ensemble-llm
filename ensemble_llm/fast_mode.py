...
class FastModeOrchestrator:
    ...
    async def race_strategy(
        self, query_func, models: List[str], prompt: str, min_responses: int = 1
    ) -> List[Dict]:
    ...
    async def cascade_strategy(
        self, query_func, models: List[str], prompt: str, cascade_delay: float = 2.0
    ) -> List[Dict]:
    ...
    async def single_best_strategy(
        self, query_func, model: str, prompt: str
    ) -> List[Dict]:
        """Use only the single best model"""
        result = await query_func(model, prompt)
        return [result] if result["success"] else []

    async def quick_consensus_strategy(
        self, query_func, models: List[str], prompt: str
    ) -> List[Dict]:
...
class TurboMode:
    ...
    async def turbo_query(
        self, model: str, prompt: str, session: Optional[aiohttp.ClientSession] = None
    ) -> Dict:
